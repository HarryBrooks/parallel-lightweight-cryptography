...
void pLayer(BYTE* state)
{
	...
	int nBitsOver4 = nBits / 4;
	int nBitsMinus1 = nBits - 1;
	
	for(int i = 0; i < nSBox-1; i++){
		int permutedBitNos[8];
		BYTE yValues[8];
		BYTE xValues[8];
		for(int j = 0; j < 8; j++) {
			permutedBitNos[j] = ((i*8+j)*nBitsOver4)%nBitsMinus1;
			yValues[j]			= permutedBitNos[j]/8;
			xValues[j]			= GET_BIT(state[i],j);
		}
		for(int j = 0; j < 8; j++){ 
			tmp[yValues[j]]		^= xValues[j] << (permutedBitNos[j] - 8*yValues[j]);
		}
	}
