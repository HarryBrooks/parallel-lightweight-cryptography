	...
	const size_t iolen_inbytes)
{
	...
	// Vectorised - quite a fast speed up too
	// State[i] is aligned, in_right[i] is unaligned
	#pragma omp simd
	...
	const size_t iolen_inbytes)
{
	...
	// Not vectorised, we have a lot of unaligned access but it is possible
	#pragma omp simd
	...
	const size_t DBlen_inbytes)
{
	...
	size_t iteration1 = DBlen_inbytes < RATE_INBYTES / 2 ? DBlen_inbytes : RATE_INBYTES / 2;
	...
	#pragma omp simd
	for (i = iteration1; i < DBlen_inbytes; i++) {
	...
	const uint32_t EncDecInd)
{
	...
	// The primary loop they have used, these guys clearly like functional programming; it isn't easy
	// to read this code.
	// 8/1/20 - This doesn't work for all circumstances, need a way to turn parallel off for very small.
	...
	for (i = 0; i < Dlen_inblocks - 1; i++)
	{
		...
		// Terrible function name, but it appears to xor
