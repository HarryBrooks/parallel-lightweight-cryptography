...
const uint32_t TO_ADD[4] = {KS_CONSTANT_0, KS_CONSTANT_1, KS_CONSTANT_2, KS_CONSTANT_3};
const uint32_t COL_M[5] = {COL_M0, COL_M1, COL_M2, COL_M3, COL_MK};
const uint32_t COL_INV_M[4] = {COL_INV_M0, COL_INV_M1, COL_INV_M2, COL_INV_M3};
//#define TO_ADD  {KS_CONSTANT_0, KS_CONSTANT_1, KS_CONSTANT_2, KS_CONSTANT_3}
...
{
    int i;
    // 16/1/20 - Not vectorised, assumed dependency | Sometimes it doesn't vectorise due to bad efficiency
    #pragma omp simd
...
uint32_t mat_mult(uint32_t mat_col, uint32_t vec)
{
    int i;
    uint32_t mask, res=0;
    // 16/1/20 - Not vectorised, assumed dependency
    // 17/1/20 - Dependency exists, I don't know how to speed this one up
    for (i = 31; i>=0; i--)
    {
        mask = -((vec >> i) & 1);
        res ^= mask & mat_col;
...
void key_schedule(const uint8_t *key, uint32_t* round_keys)
{
    ...
    // 16/1/20 - Not vectorised, suggests SIMD directive
    // 17/1/20 - A dependency exists here - attempting large loop alteration which might work
    
    for (r=0; r<NB_ROUNDS_KS; r++)
    {
        ks_state += 4;

        // Dependent on the previous iteration
        ks_mix_comlumns(ks_state-4, ks_state);

        // These two are ok, but given the dependency above I don't think we can get away with parallelism
...
// 17/1/20 - I don't think we can vectorise because of embeeded loop
...
void mix_rows_128(uint32_t *state)
{
    ...
    */
    for(int i = 0; i < 4; i++) {
        state[i] = mat_mult(COL_M[i], state[i]);
    }
}
...
// 16/1/20 - This function appears to be a source of interest for the compiler. This as a loop may work.
...
void inv_mix_rows_128(uint32_t *state)
{
    ...
    */
    #pragma omp simd
    for(int i = 0; i < 4; i++) {
        state[i] = mat_mult(COL_INV_M[i], state[i]);
    }
}
