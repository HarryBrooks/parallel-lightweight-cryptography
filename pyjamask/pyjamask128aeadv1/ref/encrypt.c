...
static void xor_block(block d, block s1, block s2) {
    ...
    // 16/1/20 - Loop vectorised 22.4x speed up | Memory aligned
    #pragma omp simd
    for (i=0; i<16; i++)
        d[i] = s1[i] ^ s2[i];
}
...
static void double_block(block d, block s) {
    ...
    // 16/1/20 - Loop vectorised 1.5x speed up | Memory unaligned
    #pragma omp simd
    for (i=0; i<15; i++)
        d[i] = (s[i] << 1) | (s[i+1] >> 7);
...
static void calc_L_i(block l, block ldollar, unsigned i) {
    ...
    // 16/1/20 - Loop Vectorised 1.4x speed up | Memory unaligned
    // 17/1/20 - Cannot add simd here due to odd condition
    for ( ; (i&1)==0 ; i>>=1)
...
static int ocb_crypt(unsigned char *out, unsigned char *k, unsigned char *n,
    ...
    bitshift  = bottom%8;
    if (bitshift != 0)
        // 16/1/20 - Loop not vectorised | Inefficient | Memory unaligned and irregularly load
        for (i=0; i<16; i++)
    ...
    unsigned char * originalIn = in;
    unsigned char * originalOut = out;
    //unsigned char newTemps[inbytes/16][16];
    //unsigned char offsets[inbytes/16][16];

    
    unsigned char** newTemps = (unsigned char**) malloc(inbytes / 16 * sizeof(unsigned char*));
    unsigned char** offsets = (unsigned char**) malloc(inbytes / 16 * sizeof(unsigned char*));
    for(i=0;i<inbytes/16;i++){
        newTemps[i] = (unsigned char*) malloc(16 * sizeof(unsigned char));
        offsets[i] = (unsigned char*) malloc(16 * sizeof(unsigned char));
    }
    
    for (i=1; i<=inbytes/16; i++) {
        block newTemp;
        calc_L_i(newTemp, ldollar, i);
        xor_block(offset, offset, newTemp);
        for(int j=0;j<16;j++) {
            newTemps[i-1][j] = newTemp[j];
            offsets[i-1][j] = offset[j];
        }
    }
    ...
    #pragma omp parallel
    {
        #pragma omp for
        for (i=1; i<=inbytes/16; i++) { 
            unsigned char * currentin = originalIn + (i-1) * 16;
            unsigned char * currentout = originalOut + (i-1) * 16;
            ...
            xor_block(offset, offset, newTemp);
            */
            block newTemp, localOffset;
            for(int j=0;j<16;j++){
                newTemp[j] = newTemps[i-1][j];
                localOffset[j] = offsets[i-1][j];
            }

            xor_block(newTemp, localOffset, currentin);

            if (encrypting) {
                /* Checksum_i = Checksum_{i-1} xor P_i */
                //#pragma omp critical
                //xor_block(sum, currentin, sum);
                /* C_i = Offset_i xor ENCIPHER(K, P_i xor Offset_i) */
                pyjamask_128_enc(newTemp, k, newTemp);
                
                xor_block(currentout, localOffset, newTemp);

                for(int j=0;j<16;j++) {
                    newTemps[i-1][j] = currentin[j];
                }
            } else {
                
                /* P_i = Offset_i xor DECIPHER(K, C_i xor Offset_i) */
                pyjamask_128_dec(newTemp, k, newTemp);
                
                xor_block(currentout, localOffset, newTemp);
                /* Checksum_i = Checksum_{i-1} xor P_i */
                //#pragma omp critical
                //xor_block(sum, currentout, sum);
                for(int j=0;j<16;j++) {
                    newTemps[i-1][j] = currentout[j];
                }
            }
        }
    }
    if(i > 1)
    {
        in = originalIn + (i-1) * 16;
        out = originalOut + (i-1) * 16;
    }

    // Process the xor that I didnt do
    for (i=1; i<=inbytes/16; i++) {
        for (int j=0; j<16; j++) {
            sum[j] ^= newTemps[i-1][j];
        }
    }
