			...
			)
{
	...
	// 10/1/20 - Loop was vectorised but state has unaligned access
	...
	{
		//Assigning nonce
		// 10/1/20 - Loop was not vectorised (seems inefficient). Memory unaligned and irregualar accesses
		for ( i = 0; i < 16; i++ )
			state[rate_bytes256[i]] = npub[i];

		//Initial assignment of Key
		// 10/2/20 - Loop was not vectorised (seems inefficient). Memory unaligned and irregualar accesses
		for ( i = 0; i < 16; i++ )
		     ...
		     )
{
	...
	for ( j = 0; j < ad128len; j++ )
	{
		...
		// 10/1/20 - Not vectorised due to assumed dependence
		for ( i = 0; i < 16; i++ )
		        ...
		        )
{
	...
	if ( CRYPTO_KEYBYTES == 16 && tlen == 16 )
	{
		...
		// 10/1/20 - Vectorised but unaligned accesses
		for ( i = 0; i < 8; i++ )
		{
			tag[i] = state[8+i];
			tag[8+i] = state[24+i];
			...
			)
{
	...
	unsigned char state[sizeof(unsigned char)*STATEBYTES];
	unsigned char tag[sizeof(unsigned char)*CRYPTO_ABYTES];
	...
	if ( mlen != 0 )
	{
		for ( j = 0; j < m128len; j++ )
		{
			...
			// 10/1/20 - Not vectorised, assumed dependence
			for ( i = 0; i < 16; i++ )
			{
	...
	if ( spoc128_gentag( tag, CRYPTO_ABYTES, state ) != KAT_SUCCESS )
        ...
        {		
				// 10/1/20 - Vectorised but unalgined accesses
